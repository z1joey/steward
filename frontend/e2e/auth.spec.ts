...
import { createStore, PASSWORD, registerAndLogin, uniqueEmail, uiLogin } from "./helpers";

test("T-AUTH-01 · 注册 → 自动登录落引导；错误凭证提示（AC-AUTH-01/02）", async ({ page }) => {
  const email = uniqueEmail("e2e-a1-");
...
test("AC-AUTH-01 · 重复邮箱注册拒绝（AC-AUTH-01）", async ({ page, request }) => {
  const email = uniqueEmail("e2e-dup-");
  await registerAndLogin(request, email);   // 先占号
...
test("T-AUTH-02 · 无店仅「创建门店」与待处理邀请（AC-AUTH-03）", async ({ page, request }) => {
  const actor = await registerAndLogin(request, uniqueEmail("e2e-a2-"));
  await uiLogin(page, actor.email);
...
test("T-AUTH-03 · 创建门店 → 进流水；单店隐藏选择器（AC-AUTH-04/05）", async ({ page }) => {
  const email = uniqueEmail("e2e-a3-");
  const storeName = `司舵e2e-${email.slice(-6)}`;
...
test("AC-AUTH-05 · 多店显示选择器且仅列已接受门店", async ({ page, request }) => {
  const actor = await registerAndLogin(request, uniqueEmail("e2e-a5-"));
  const nameA = `店A-${uniqueEmail("")}`;
  const nameB = `店B-${uniqueEmail("")}`;
